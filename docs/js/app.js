    ...
    map.on("click", lid, (e) => {
      ...
      campPopup = new mapboxgl.Popup({ closeButton: true, maxWidth: "240px" })
        ...
        .setHTML(
          ...
          (fee  ? `<span style="color:#ffd700">Fee: ${fee}</span><br>` : "") +
          (reserveUrl
...
async function loadCampgroundData() {
  ...
  // Read pre-built static GeoJSON generated by scripts/build_campgrounds.mjs
  // (Recreation.gov RIDB API — NPS + National Forest campgrounds, with fees + reservation links)
  try {
    const res = await fetch("./data/campgrounds.json");
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const geojson = await res.json();

    campgroundCache = geojson;

    // Update count badge
    const countEl = document.getElementById("count-campgrounds");
    if (countEl) countEl.textContent = `(${geojson.features?.length ?? 0})`;
